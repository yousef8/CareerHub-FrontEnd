<template>
  <div v-if="jobPost" class="card mb-3">
    ...
    <div class="card-footer d-flex justify-content-end">
      ...
      <button @click="rejectJobPost" class="btn btn-danger"><i class="bi bi-x"></i> Reject</button>
    </div>
  </div>
...
export default defineComponent({
  ...
  computed: {
    jobPost(): JobPost | undefined {
      return this.jobPostStore.getPendingJobPostById(this.jobPostId)
    }
  },
  methods: {
    approveJobPost() {
    ...
    rejectJobPost() {
      this.jobPostStore.rejectJobPost(this.jobPostId)
    },
