  ...
  <div class="mb-5 border border-3 p-4 border-secondary rounded">
    ...
    <div v-else>
      <div v-for="(application, index) in applications" :key="index" class="card mb-3" >
        <div class="card-body">
          <h5 class="card-title"><span class="text-primary">Application ID: </span> {{ application.id }}</h5>
          <div class="row justify-content-center align-items-center">
            <div class="col-auto">
                <p class="card-text fs-5 fw-bold"><span class="text-primary">Job Title: </span> {{ application.job_post.title }}</p>
            </div>
            <div class="col-auto">
                <router-link :to="`/jobs/${application.job_post.id}`" class="btn btn-primary"><i class="fas fa-eye"></i></router-link>
            </div>
          </div>
